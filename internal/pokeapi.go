...
func Areas(url string) (Area, error) {
	areas := Area{}
...
func Catch(url string) (Pokemon, error) {
	pokemon := Pokemon{}
	data, hit := cache.Get(url)
	if hit {
		...
		if res.StatusCode > 299 {
			return Pokemon{}, fmt.Errorf("response failed with status: %v", res.StatusCode)
		}
...
func Locations(url string) (Location, error) {
	...
	if hit {
		...
		if res.StatusCode > 299 {
			return Location{}, fmt.Errorf("response failed with status: %v", res.StatusCode)
		}
